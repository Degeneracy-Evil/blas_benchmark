...
BenchmarkConfig ConfigParser::parse_string(const std::string& content) const
{
    ...
    try
    {
        ...
        if(tbl.contains("defaults"))
        {
            ...
            if(defaults.as_table()->contains("level2_m") &&
               defaults.as_table()->contains("level2_n"))
            {
                config.level2_size = {defaults["level2_m"].value_or(1024),
                                      defaults["level2_n"].value_or(1024)};
            }
            if(defaults.as_table()->contains("level3_m") &&
               defaults.as_table()->contains("level3_n") &&
               defaults.as_table()->contains("level3_k"))
            {
                config.level3_size = {defaults["level3_m"].value_or(1024),
