...
std::optional<std::tuple<int, int, int>> parse_size_triple(const std::string& str)
{
    auto pos1 = str.find(',');
...
void print_system_info(const blas_benchmark::utils::SystemInfo& info)
{
    ...
    std::println("Cores:        {} physical, {} logical", info.physical_cores, info.cpu_cores);
...
int main(int argc, char* argv[]) // NOLINT(bugprone-exception-escape)
{
    ...
    app.add_option("-c,--cycle", cycles, "Number of benchmark cycles")->default_val(5);
    app.add_option("-w,--warmup", warmup, "Number of warmup iterations")->default_val(3);
    ...
    app.add_option("-o,--output", output_file, "Output file path")->default_val("");
    app.add_option("-f,--format", format, "Output format (markdown|csv)")->default_val("markdown");
    app.add_option("-C,--config", config_file, "Configuration file path")
        ->default_val("config.toml");
    app.add_flag("-v,--verbose", verbose, "Enable verbose output");
    app.add_flag("-s,--system-info", show_system_info, "Show system information only");
    ...
    if(std::filesystem::exists(config_file))
    {
        ...
        {
            spdlog::warn("Failed to load config file: {}. Using defaults.", e.what());
    ...
    if(!level2_str.empty())
    {
        ...
        {
            spdlog::error("Invalid level2 size format: {}. Expected M,N", level2_str);
            return 1;
        }
    }
    ...
    if(!level3_str.empty())
    {
        ...
        {
            spdlog::error("Invalid level3 size format: {}. Expected M,N,K", level3_str);
            return 1;
        }
    }
    ...
    try
    {
        ...
        std::string output = blas_benchmark::OutputFormatter::format(report, config.format);
