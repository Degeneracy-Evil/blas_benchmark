...
template <typename T> struct BlasPrecisionTraits;

template <> struct BlasPrecisionTraits<double>
{
...
template <> struct BlasPrecisionTraits<float>
{
...
constexpr std::size_t dot(std::size_t n) { return 2 * n; }

// daxpy: n multiplications + n additions = 2n FLOPs
constexpr std::size_t axpy(std::size_t n) { return 2 * n; }

// dscal: n multiplications = n FLOPs
constexpr std::size_t scal(std::size_t n) { return n; }

// Level 2 operations
// dgemv: m*n multiplications + m*(n-1) additions ≈ 2mn FLOPs
constexpr std::size_t gemv(std::size_t m, std::size_t n) { return 2 * m * n; }

// Level 3 operations
// dgemm: m*n*k multiplications + m*n*(k-1) additions ≈ 2mnk FLOPs
constexpr std::size_t gemm(std::size_t m, std::size_t n, std::size_t k) { return 2 * m * n * k; }

} // namespace flops

// BLAS function wrapper with template support for precision
template <typename T = double> class BlasWrapper
{
public:
    ...
    static void gemv(CBLAS_ORDER order, CBLAS_TRANSPOSE trans, std::size_t m, std::size_t n,
                     T alpha, const T* a, int lda, const T* x, int incx, T beta, T* y, int incy)
    {
        if constexpr(std::is_same_v<T, double>)
        {
            cblas_dgemv(order, trans, static_cast<int>(m), static_cast<int>(n), alpha, a, lda, x,
                        incx, beta, y, incy);
        }
        else
        {
            cblas_sgemv(order, trans, static_cast<int>(m), static_cast<int>(n), alpha, a, lda, x,
                        incx, beta, y, incy);
        }
    }
    ...
    static void gemm(CBLAS_ORDER order, CBLAS_TRANSPOSE trans_a, CBLAS_TRANSPOSE trans_b,
                     std::size_t m, std::size_t n, std::size_t k, T alpha, const T* a, int lda,
                     const T* b, int ldb, T beta, T* c, int ldc)
    {
        if constexpr(std::is_same_v<T, double>)
        {
            cblas_dgemm(order, trans_a, trans_b, static_cast<int>(m), static_cast<int>(n),
                        static_cast<int>(k), alpha, a, lda, b, ldb, beta, c, ldc);
        }
        else
        {
            cblas_sgemm(order, trans_a, trans_b, static_cast<int>(m), static_cast<int>(n),
                        static_cast<int>(k), alpha, a, lda, b, ldb, beta, c, ldc);
        }
    }
};
...
double benchmark_dot(std::size_t n, std::size_t warmup, std::size_t cycles, bool flush_cache,
                     std::size_t cache_size);

template <typename T = double>
double benchmark_axpy(std::size_t n, std::size_t warmup, std::size_t cycles, bool flush_cache,
                      std::size_t cache_size);

template <typename T = double>
double benchmark_scal(std::size_t n, std::size_t warmup, std::size_t cycles, bool flush_cache,
                      std::size_t cache_size);
...
template <typename T = double>
double benchmark_gemm(std::size_t m, std::size_t n, std::size_t k, std::size_t warmup,
                      std::size_t cycles, bool flush_cache, std::size_t cache_size);

// Benchmark function signature
template <typename T = double>
using BenchmarkFunc = std::function<double(std::size_t warmup, std::size_t cycles, bool flush_cache,
                                           std::size_t cache_size)>;
